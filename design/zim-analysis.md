...
|---------|------|----------|----------|
| `wikipedia_en_simple_all_maxi` | 3.1G | ~240K | 16GB tier, learners, non-native speakers |
| `wikipedia_en_top_maxi` | 7.6G | ~50K | Quick reference with images |
| `wikipedia_en_all_mini` | 11G | 6.9M+ | 32GB tier, full coverage compressed |
| `wikipedia_en_all_nopic` | 48G | 6.9M+ | 64GB tier, full text |
| `wikipedia_en_all_maxi` | 115G | 6.9M+ | Max tier only |

**Decision**: Simple English for 16GB, mini for 32GB, nopic for 64GB, maxi
for Max. Simple English is underrated — it's written at an accessible reading
level, making it ideal for non-native speakers and young readers.
...
references). A 16GB kit with medical guides, survival content, Simple
...
| Tails (img + iso) | 3.0 GB | 32, 64, max |
| OnionShare (all platforms) | 200 MB | 32, 64, max |
| Orbot | 50 MB | all |
| Kiwix tools + desktop | 100 MB | 32, 64, max |
...
|------|-------------|-----------------|-------------------|
| 16 GB | 14 GB | ~1 GB | **~13 GB** |
| 32 GB | 29 GB | ~4.5 GB | **~24.5 GB** |
| 64 GB | 58 GB | ~4.5 GB | **~53.5 GB** |
| Max | unlimited | ~4.5 GB | **unlimited** |

Note: 16GB tier currently has no Kiwix tools/desktop. ZIM files at this tier
would need to be read on a computer that already has Kiwix installed, or we
would need to add kiwix-tools to the 16GB tier (~50 MB).
...
### P0 — Critical (include in ALL tiers)
...
| 2 | `wikem_en_all_maxi_2021-02.zim` | 42 MB | Medical | Emergency medicine wiki (WikEM) |
...
### P1 — High priority (include starting at 16 GB tier)

Core reference and education content to fill the 16GB budget. Combined with
P0, targets ~13 GB.
...
**P0 + P1 subtotal: ~12.2 GB** (fits in 16GB tier's 13 GB ZIM budget with margin)

### P2 — Medium priority (include starting at 32 GB tier)
...
**32GB tier note**: At this tier, `wikipedia_en_all_mini` (11G) replaces
...
### P3 — Extended (include starting at 64 GB tier)

Comprehensive references and richer educational content. Combined with
P0+P1+P2, targets ~53.5 GB.
...
**64GB tier note**: At this tier, `wikipedia_en_all_nopic` (48G) replaces
`wikipedia_en_all_mini` (11G), net add ~37G. `wikibooks_en_all_maxi` (5.1G)
replaces nopic (2.9G), net add ~2.2G. The 64GB tier can't fit everything in
P3 — prioritize Wikipedia nopic + Wiktionary + one TED collection + one
Gutenberg collection, selecting based on remaining space. Estimated fit:
...
### 16 GB tier (~13 GB ZIM budget)

| Priority | Files | ZIM total |
|----------|-------|-----------|
| P0 | Items 1-10 | 1.04 GB |
| P1 | Items 11-17 | 11.2 GB |
| **Total** | **17 ZIM files** | **~12.2 GB** |

Margin: ~0.8 GB. Consider adding kiwix-tools (~50 MB) to this tier if ZIM
files are included.

### 32 GB tier (~24.5 GB ZIM budget)
...
### 64 GB tier (~53.5 GB ZIM budget)

This tier replaces Wikipedia mini with nopic (+37 GB net), making it tight.
Recommended allocation:

| Priority | Files | ZIM total |
|----------|-------|-----------|
| P0 | Items 1-10 | 1.04 GB |
| P1 | Items 12-17 (minus Simple Wiki) | 7.9 GB |
| P2 | Items 19-23 (minus Wikipedia mini, replaced) | 15.1 GB |
| P3 core | Item 24 (Wikipedia nopic) | 48 GB |
| **Total** | | **~52 GB** |

Margin: ~1.5 GB — very tight. Adding Wiktionary (8.2 GB) or TED Science
(13 GB) would exceed budget. Option: drop some P1/P2 items to make room for
Wiktionary, or keep the margin for growth.

**Alternative 64 GB allocation** (if Wikipedia nopic is too dominant):
Keep Wikipedia mini (11G), add Wiktionary (8.2G), add TED Science (13G),
add more Gutenberg collections. This gives broader coverage at the cost of
Wikipedia depth.

### Max tier (unlimited)

Include everything from P0-P3 plus all P4 items. At minimum:
- Wikipedia full maxi (115G)
- Stack Overflow (75G)
- Khan Academy (168G)
- Complete Gutenberg (206G)
- All TED talks (79G)
- Crash Course (21G)
- Multiple language Wikipedias

Total Max tier ZIM: ~700+ GB depending on language coverage.
...
Kiwix tools and desktop are already defined as sources (`kiwix-tools`,
`kiwix-desktop`) in tiers 32+. For the 16GB tier, if ZIM files are included,
`kiwix-tools` should be added to that tier's source list as well.
...
1. **Should the 16GB tier include kiwix-tools?** Without it, users need
   pre-installed Kiwix software to read ZIM files. Adding it costs ~50 MB.

2. **Language profiles**: Should OAK support language-variant builds (e.g.,
   an Arabic-focused 32GB tier)? This would change the ZIM selection
   significantly.

3. **Update frequency**: ZIM files are updated monthly to quarterly. Should
   OAK pin specific versions or always fetch latest?

4. **64GB Wikipedia tradeoff**: Is it better to have Wikipedia nopic (48G,
   all articles, no images) or Wikipedia mini (11G, compressed) plus many
